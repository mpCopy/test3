...
if ($mode == 1)
{
	...
	print MASTERFILE "load(strcat(expandenv(\"\$HPEESOF_DIR\"),\n\tapi_get_directory_delimiter(),\"CustomEncoded\",\n\tapi_get_directory_delimiter(),\"ael\",\n\tapi_get_directory_delimiter(),\"$defFile\"));\n";
	close(MASTERFILE);
        ...
        print NewMenu $line;
        while (($line = <OldMenu>) && !(($line =~ /winT/) && ($line =~ /SCHEMATIC_WINDOW/)))
        {
	    print NewMenu $line;
        }
        print NewMenu $line;
