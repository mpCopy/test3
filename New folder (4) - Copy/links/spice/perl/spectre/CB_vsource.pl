...
###############################################################################
#
# This callback will resolve the vsource type(dc,sine,pulse,pwl,exp)
#
#    vsource, type=dc -> V_Source,  Vdc
#    vsource, type=sine ->V_Source, V_Tone=damped_sin(time,offset,amplitude,freq,delay,damping)
#    vsource, type=pulse ->V_Source, V_Tone=pulse(time, low, high, delay, rise, fall, width, period)
#    vsource, type=exp ->V_Source, V_Tran=exp_pulse(time,low, high, tdelay1,tau1, tdelay2, tau2)
#    vsource, type=pwl ->V_Source, V_Tran=pwl(time,t1,v1,t2,v2...tn,vn)
#  
###############################################################################
...
sub cb_vsource_type
{
   ...
   my $paramPhaseVal="";
   my $DelayVal=0;
   #Variables for sine vsource
   my $AmpVal=1;   my $FreqVal="";    my $DampVal=0;
   
   #Variables for pulse vsource
   ...
   if($typeVal eq "sine")
   {
	...
	{  
	   $numP++;
    	   $SpiceCktT[$tmpCKT]->{elementArray}[$x]->{paramArray}[$numP]{name} ="Vac";
	...
	#Processing V_Tran=damped_sin(time,offset,amplitude,freq,delay,damping)
	$numP++;
	$SpiceCktT[$tmpCKT]->{elementArray}[$x]->{paramArray}[$numP]{name} ="V_Tran";
	...
	#Processing V_Tran=pulse(time, low, high, delay, rise, fall, width, period) 
	...
	#Processing V_Tran=exp_pulse(time,low, high, tdelay1,tau1, tdelay2, tau2)
	$numP++;
	$SpiceCktT[$tmpCKT]->{elementArray}[$x]->{paramArray}[$numP]{name} ="V_Tran";
    ...
    {  
       ...
       {	
         ...
         {
           $SpiceCktT[$tmpCKT]->{elementArray}[$x]->{paramArray}[$y]{name}="V_Tran";
