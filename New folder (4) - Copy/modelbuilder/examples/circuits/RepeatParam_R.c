...
LOCAL const char sccs_cui_circuit_template[]="@(#) $Source: /cvs/sr/src/geminiui/modelbuilder/RepeatParam_R.c,v $ $Revision: 1.4 $ $Date: 2011/08/27 00:17:20 $";
...
using namespace UCM_RepeatParam;
...
MyParam::setResValues( const UserInstDef* userInst, int paramIndex )
{
    ...
    {
        // get repeated parameter values: Res[1], Res[2], ....
        int numRepeats = get_ucm_param_num_repeats( userInst, paramIndex );
        mResValues.resize( numRepeats );

        for( int repeatIndex = 0; repeatIndex < numRepeats; repeatIndex++ )
        {
            char errorMsg[2048];
            // get pointer to the parameter based on repeatIndex
