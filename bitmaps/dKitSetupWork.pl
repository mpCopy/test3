...
use lib "$ENV{DKITVERIFICATION}/bin";

#run dKitTemplateCreate.pl, dKitParameterCreate.pl and dKitSubcircuitCreate.pl'
require 'dKitTemplateCreate.pl';
require 'dKitParameterCreate.pl';
require 'dKitSubcircuitCreate.pl';

use File::Copy;

if ( ! -e "header.pl" )
{
    copy("$ENV{DKITVERIFICATION}/perl/header.pl", "header.pl");
} else
{
    print "Left existing header file header.pl unaltered\n"
}
