...
# S-parameter test for npn having 3 or more pins. 
# This template replaces npn3_sparam.pl, npn4_sparam.pl, npn3_subckt_sparam.pl, 
# and npn4_subckt_sparam.pl.
# The mverify script is changed to accomodate for tests created for npn3_subckt_sparam
# and npn4_subckt_sparam. These tests will invoke this template with the following
# options: 
#          -num_pins=3/4/...
#          -prefix=q/x
...
$ret = GetOptions ("h|help", "delfiles|delfiles:s", "simulators|simulators:s", "resultfile|resultfile:s", "s11_tol|s11_tol:s", "s21_tol|s21_tol:s", "s22_tol|s22_tol:s", "s12_tol|s12_tol:s", "abs_tol|abs_tol:s", "testname|testname:s", "prefix|prefix:s", "model_name|device_name:s", "allparams|allparams:s", "adsparams|adsparams:s", "hspiceparams|hspiceparams:s", "spectreparams|spectreparams:s", "emitter_v|emitter_v:f", "subs_v|subs_v:f", "base_refz|base_refz:f", "collector_refz|collector_refz:f", "base_bias|base_bias:f", "collector_bias|collector_bias:f", "freq_pts|freq_pts:s", "freq_start|freq_start:f", "freq_stop|freq_stop:f", "freq_numpts|freq_numpts:i", "sweep_vb|sweep_vb:s", "vb_pts|vb_pts:s", "vb_start|vb_start:f", "vb_stop|vb_stop:f", "vb_numpts|vb_numpts:i", "sweep_temp|sweep_temp:s", "temp_pts|temp_pts:s", "num_pins|num_pins:i", "ads_model_lib|ads_model_lib:s", "hspice_model_lib|hspice_model_lib:s", "spectre_model_lib|spectre_model_lib:s","inline|inline:s","reltol|reltol:f","vabstol|vabstol:f","iabstol|iabstol:f","gmin|gmin:f","scale|scale:f","temp|temp:f","tnom|tnom:f","digits|digits:i","ads_scale|ads_scale:f");
...
# tool. It is an S-Parameter Simulation using a 3-pin npn subcircuit.

# This simulation is intended to find S parameters vs. frequency while sweeping ib.  
...
# Add instances of BJT and three voltage/current sources, vc, vs, and ib.
...
#  print "SWEEP_LEVEL = $sweep_level\n";
  ...
  {
    if (! defined $vb_start) { $vb_start='0.5';}
    if (! defined $vb_stop) { $vb_stop='0.9';}
