...
# S-Parameter test template for pmos with 3 or more pins.
# This template replaces pmos3_subckt_sparam.pl and pmos4_subckt_sparam.pl.
# The mverify script has been changed to accomodate for tests created for
# pmos3_subckt_sparam and pmos4_subckt_sparam.
...
$s11_tol            = $opt_s11_tol;
if(!$s11_tol) {$s11_tol=1;}

$s21_tol            = $opt_s21_tol;
if(!$s21_tol) {$s21_tol=1;}

$s22_tol            = $opt_s22_tol;
if(!$s22_tol) {$s22_tol=1;}

$s12_tol            = $opt_s12_tol;
if(!$s12_tol) {$s12_tol=1;}
...
# tool. It is an S-Parameter Simulation using a MOSFET device.
