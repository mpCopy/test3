...
for $Stage (1..$numStages)
{
    ...
    $pmosDevice[$Stage]->parameterValue('modelName', $pmodel);
    if ($p_width) {$pmosDevice[$Stage]->parameterValue('width', $p_width);}
    if ($p_length) {$pmosDevice[$Stage]->parameterValue('length', $p_length);}
    if ($p_ad) {$pmosDevice[$Stage]->parameterValue(drainDiffusionArea, $p_ad);}
    if ($p_pd) {$pmosDevice[$Stage]->parameterValue(drainPerimeter,  $p_pd);}
    if ($p_nrd) {$pmosDevice[$Stage]->parameterValue(drainDiffusionSquares, $p_nrd);}
    if ($p_as) {$pmosDevice[$Stage]->parameterValue(sourceDiffusionArea, $p_as);}
    if ($p_ps) {$pmosDevice[$Stage]->parameterValue(sourcePerimeter, $p_ps);}
    if ($p_nrs) {$pmosDevice[$Stage]->parameterValue(sourceDiffusionSquares, $p_nrs);}
    ...
    $nmosDevice[$Stage]->parameterValue('modelName', $nmodel);
    if ($n_width) {$nmosDevice[$Stage]->parameterValue('width', $n_width);}
    if ($n_length) {$nmosDevice[$Stage]->parameterValue('length', $n_length);}
    if ($n_ad) {$nmosDevice[$Stage]->parameterValue(drainDiffusionArea, $n_ad);}
    if ($n_pd) {$nmosDevice[$Stage]->parameterValue(drainPerimeter, $n_pd);}
    if ($n_nrd) {$nmosDevice[$Stage]->parameterValue(drainDiffusionSquares, $n_nrd);}
    if ($n_as) {$nmosDevice[$Stage]->parameterValue(sourceDiffusionArea, $n_as);}
    if ($n_ps) {$nmosDevice[$Stage]->parameterValue(sourcePerimeter, $n_ps);}
    if ($n_nrs) {$nmosDevice[$Stage]->parameterValue(sourceDiffusionSquares, $n_nrs);}
}
...
my $passFail=mVerifyResults->ringOscResults("$testName",$testName,$resultHtmlFile,$simulators,$savePrefix,$vdd/2,1,1,$abs_tol,$tol);
print $passfail;
