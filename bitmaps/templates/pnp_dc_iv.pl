...
# DC IV test for pnp having 3 or more pins.
# This template replaces pnp3_dc_iv.pl, pnp4_dc_iv.pl, and pnp_subckt_dc_iv.pl.
...
sub Usage()
{
  ...
  print "                            -ve_pts []   -ve_start []    -ve_stop []    -ve_numpts []\n";
...
$ret = GetOptions ("h|help", "delfiles|delfiles:s", "simulators|simulators:s", "resultfile|resultfile:s", "ic_tol|ic_tol:s", "abs_tol|abs_tol:s", "testname|testname:s", "prefix|prefix:s", "model_name|device_name:s", "allparams|allparams:s", "adsparams|adsparams:s", "hspiceparams|hspiceparams:s", "spectreparams|spectreparams:s", "emitter_v|emitter_v:f", "subs_v|subs_v:f", "base_bias|base_bias:f", "collector_bias|collector_bias:f", "vc_pts|vc_pts:s", "vc_start|vc_start:f", "vc_stop|vc_stop:f", "vc_numpts|vc_numpts:i", "ve_pts|ve_pts:s", "ve_start|ve_start:f", "ve_stop|ve_stop:f", "ve_numpts|ve_numpts:i", "sweep_ib|sweep_ib:s", "ib_pts|ib_pts:s", "ib_start|ib_start:f", "ib_stop|ib_stop:f", "ib_numpts|ib_numpts:i", "sweep_l|sweep_l:s", "l_pts|l_pts:s", "l_start|l_start:f", "l_stop|l_stop:f", "l_numpts|l_numpts:i", "sweep_w|sweep_w:s", "w_pts|w_pts:s", "w_start|w_start:f", "w_stop|w_stop:f", "w_numpts|w_numpts:i", "sweep_temp|sweep_temp:s", "temp_pts|temp_pts:s", "num_pins|num_pins:i", "sweep_fingers|sweep_fingers:s", "fingers_pts|fingers_pts:s", "ads_model_lib|ads_model_lib:s", "hspice_model_lib|hspice_model_lib:s", "spectre_model_lib|spectre_model_lib:s","inline|inline:s","reltol|reltol:f","vabstol|vabstol:f","iabstol|iabstol:f","gmin|gmin:f","scale|scale:f","temp|temp:f","tnom|tnom:f","digits|digits:i","ads_scale|ads_scale:f");
...
$ve_pts             = $opt_ve_pts;
if($ve_pts) { $ve_pts = &dKitUtils::sortInAscendingOrder($ve_pts); }
$ve_start           = $opt_ve_start;
$ve_stop            = $opt_ve_stop;
if ($ve_start > $ve_stop)
{
  $tmp = $ve_stop;
...
# tool. It is an DC-IV Simulation using a pnp BJT subcircuit.
...
# Add a pnp BJT instance and three voltage/current sources, vc, ve, and ib.
...
$ib->nodeName(nminus, 0);
$ib->nodeName(nplus, 'base');
...
# Add Sweep Plans  
if ((defined $vc_pts || defined $vc_start || defined $vc_stop || defined $vc_numpts)
     && (defined $ve_pts || defined $ve_start || defined $ve_stop || defined $ve_numpts))
{
  print "\nERROR: You can define sweep points for vc or ve, but not both!!!\n\n"; exit -1;
}
if ($vc_pts) {
  ...
  $SWEEPPLAN1 = dKitAnalysis->new(SWEEPPLAN_PT, Plan1);
  $SWEEPPLAN1->parameterValue("values", $ve_pts);
  $sweepVc=0;
} elsif ($ve_start || $ve_stop || $ve_numpts) {
  if (! defined $ve_start) { $ve_start='-5';}
  if (! defined $ve_stop) { $ve_stop='-0.1';}
  if (! defined $ve_numpts) { $ve_numpts='51';}
  $SWEEPPLAN1 = dKitAnalysis->new(SWEEPPLAN_LIN, Plan1);
  $SWEEPPLAN1->parameterValue(start, $ve_start);
  $SWEEPPLAN1->parameterValue(stop, $ve_stop);
  $SWEEPPLAN1->parameterValue(numPts, $ve_numpts);
  $sweepVc=0;
}
else
{
  if (! defined $vc_start) { $vc_start='-5';}
  if (! defined $vc_stop) { $vc_stop='-0.1';}
...
#  print "SWEEP_LEVEL = $sweep_level\n";
  ...
  if ($sweepVc==1) {
    $SWEEP3->parameterValue(device, 'vc');  
  } else {
    $SWEEP3->parameterValue(device, 've');  
  }
...
if ($sweepVc==1) {
  ...
  $OUTPUTDOPS->parameterValue(deviceParameters, "ve:i");
}
...
foreach $modelName (@model_name) {
  ...
  if ($sweepVc==1) {
    $passFail=mVerifyResults->compareResults("${head}$modelName\_dc\_iv",$testname,$modelName,"dc_iv",$resultHtmlFile,$simulators,$abs_tol,"vc.i1",$ic_tol);
  } else {
    $passFail=mVerifyResults->compareResults("${head}$modelName\_dc\_iv",$testname,$modelName,"dc_iv",$resultHtmlFile,$simulators,$abs_tol,"ve.i1",$ic_tol);
  }
