...
if (($arch =~ "Win") || ($arch =~ "CYGWIN"))
{
  ...
  $dirtoget="$hped/ComponentLibs/$libName/bitmaps";
  ...
  foreach $bmpfile ( @BMPFILES )
  {
    unless( ($bmpfile eq ".") || ($bmpfile eq "..")) 
    {
      ...
      if (substr( $bmpfile, length($bmpfile)-6, 6) eq ".win32")
      {
        ...
        file_copy("$hped/ComponentLibs/$libName/bitmaps/$bmpfile",
	  "$hped/ComponentLibs/$libName/bitmaps/$outfile.bmp");
      }
    }
    ##file_copy("$hped/ComponentLibs/$libName/bitmaps/encoded.win32", 
    ##	 "$hped/ComponentLibs/$libName/bitmaps/encoded.bmp");
    $bootscript = " ";
  }
} else
{
...
$defAEL = "$hped/ComponentLibs/ael/".$libName."_encode_def.ael";
$defATF = "$hped/ComponentLibs/ael/".$libName."_encode_def.atf";
$itemAEL = "$hped/ComponentLibs/ael/".$libName."_encode_item.ael";
$itemATF = "$hped/ComponentLibs/ael/".$libName."_encode_item.atf";
...
$aelFile = "$hped/ComponentLibs/ael/"."$libName"."_encode_item.ael";
`$bootscript $hped/bin/hpedlibgen -in $aelFile -out $hped/ComponentLibs/records/$libName.idf`;

## Modify the de_encode_master.ael file to include this libraries palette defintions
## file
#$masterFile = "$hped/ComponentLibs/ael/de_encode_master.ael";
#$defFile1 = "ComponentLibs/ael/$libName"."_encode_def";
...
#close(MASTERFILE);
#$masterFileATF = "$hped/ComponentLibs/ael/de_encode_master.atf";
...
while ($line = <ADSLIBCONFIG>)
{
    ...
    {
	print ADSLIBCONFIG2 "$libName	\$HPEESOF_DIR/ComponentLibs/models/$libName.library\n";
	$entryMade = 1;
    }
    else
    {
...
while ($line = <BROWCFG>)
{
	...
	{
	    $line = $line.";\$HPEESOF_DIR/ComponentLibs/records";
        }
    }
    print BROWCFG2 "$line\n";
}
...
while ($line = <SIMCFG>)
{
    ...
    {
       $line = $line.":\$HPEESOF_DIR/ComponentLibs/$libName/data";
    }
    print SIMCFG2 "$line\n";
}
close(SIMCFG);
