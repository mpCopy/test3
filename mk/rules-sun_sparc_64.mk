...
# win32
$(OBJPATH)/$(LIBBASENAME)$(SLLIBVER).so $(LIBBASENAME)$(SLLIBVER).exp: $(OBJS)
	$(call SL_MAKE_EXP,$(LIBBASENAME)..so,$^,$(LIBBASENAME).exp)
	$(call SL_MAKE_OBJFILES,$(OBJS))
	$(call SL_LINK_CXX,$(OBJPATH)/$(LIBBASENAME)$(SLLIBVER).so,$(OBJS),$(TARGET_LIBS),$(LIBBASENAME))
...
ifeq ($(suffix $(TARGET)),.sl)
ALL_OBJECTS += $(OBJPATH)/$(LIBBASENAME)$(SLLIBVER).so
INSTALL_OBJECTS += $(INSTALL_LIBDIR)/$(LIBBASENAME).so
else
ifeq ($(suffix $(TARGET)),.a)
