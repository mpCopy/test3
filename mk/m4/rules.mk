...
define COMPILE.c
$(CC_PREFIX) $(CC) $(CFLAGS) $(CPPFLAGS) CcCompileOption $(1) $(2) $(3)
endef

define COMPILE.cxx
$(CXX_PREFIX) $(CXX) $(CXXFLAGS) $(CPPFLAGS) CxxCompileOption $(1) $(2) $(3)
endef

%ObjSuffix: %.c
	$(call COMPILE.c, CcTarget $<)

%ObjSuffix: %.cc
	$(call COMPILE.cxx, CxxTarget $<)
...
# TARGETS!
$(OBJPATH)/$(LIBBASENAME)ArSuffix: $(OBJS)
...
# win32
$(OBJPATH)/$(LIBBASENAME)$(SLLIBVER)SlSuffix $(LIBBASENAME)$(SLLIBVER).exp: $(OBJS)
	$(call SL_MAKE_EXP,$(LIBBASENAME).SlSuffix,$^,$(LIBBASENAME).exp)
	$(call SL_MAKE_OBJFILES,$(OBJS))
	$(call SL_LINK_CXX,$(OBJPATH)/$(LIBBASENAME)$(SLLIBVER)SlSuffix,$(OBJS),$(TARGET_LIBS),$(LIBBASENAME))
...
vpath ArPattern $(LIB_SEARCH_DIRS)
vpath SlPattern $(LIB_SEARCH_DIRS)
$(EXEPREFIX)$(basename $(TARGET))ExeSuffix: $(LOCAL_LIBS:%=-l%) $(LIBS:%=-l%)

# DO NOT CHANGE THIS WITHOUT ALSO CHANGING THE SCRIPT THAT HANDLES MULTIPLE
# TARGETS!
$(EXEPREFIX)$(basename $(TARGET))ExeSuffix: $(OBJS)
...
ifeq ($(suffix $(TARGET)),.sl)
ALL_OBJECTS += $(OBJPATH)/$(LIBBASENAME)$(SLLIBVER)SlSuffix
INSTALL_OBJECTS += SlInstallObjects(SlTargetSuffix)
else
ifeq ($(suffix $(TARGET)),.a)
ALL_OBJECTS += $(OBJPATH)/$(LIBBASENAME)ArSuffix
INSTALL_OBJECTS += $(INSTALL_LIBDIR)/$(LIBBASENAME)ArSuffix
else
ifeq ($(suffix $(TARGET)),)
ifdef TARGET
ALL_OBJECTS += $(EXEPREFIX)$(basename $(TARGET))ExeSuffix
INSTALL_OBJECTS += $(INSTALL_BINDIR)/$(EXEPREFIX)$(basename $(TARGET))ExeSuffix
endif	# TARGET
...
define(`EchoVar',`@$(ECHO) $1 = $($1)')

echoenv::
	EchoVar(PROJECT_NAME)
	EchoVar(DIRS)
	EchoVar(TARGET)
	EchoVar(TARGETS)
ifdef TARGETS
	...
	EchoVar(HDRS)
	EchoVar(PUBLIC_HDRS)
	EchoVar(INSTALL_HDRS)
	EchoVar(AEL_SRCS)
	@echo
	EchoVar(ALL_OBJECTS)
	EchoVar(QUICK_REGRESS)
	EchoVar(NIGHT_REGRESS)
	EchoVar(FULL_REGRESS)
	EchoVar(INSTALL_OBJECTS)
	EchoVar(OBJS)
	@echo
	EchoVar(NONLIBSRCS)
	EchoVar(NONLIBOBJS)
	EchoVar(NONTARGETSRCS)
	EchoVar(EXTRASRCS)
	@echo
	EchoVar(SRCPATH)
	EchoVar(VPATH)
	EchoVar(PREINCLUDEPATH)
	EchoVar(LOCAL_BUILD_MIRROR_INCLUDEPATH)
	EchoVar(LOCAL_BUILD_INCLUDEPATH)
	EchoVar(EXTRA_INCLUDEPATH)
	EchoVar(INCLUDEPATH)
	EchoVar(CINCLUDEPATH)
	EchoVar(CXXINCLUDEPATH)
	EchoVar(INCLUDEPATHOPTION)
	EchoVar(LIB_SEARCH_DIRS)
	EchoVar(_LIBPATTERNS)
	EchoVar(LIBSPATHOPTION)
	EchoVar(LIBSOPTION)
	EchoVar(LIBSPATH)
	EchoVar(LIBS)
	EchoVar(LOCAL_BUILD_MIRROR_LIBSPATH)
	EchoVar(LOCAL_BUILD_LIBSPATH)
	EchoVar(LOCAL_LIBSPATH)
	EchoVar(LOCAL_LIBS)
	EchoVar(EXTRA_INCLUDEPATH)
	EchoVar(EXTRA_LIBSPATH)
	EchoVar(EXTRA_LIBS)
	EchoVar(CUSTPACKAGES)
	EchoVar(CUSTPACKAGES_FILES)
	EchoVar(DEVPACKAGES)
	EchoVar(DEVPACKAGES_FILES)
	EchoVar(SPECIAL_CUSTPACKAGES)
	EchoVar(SPECIAL_DEVPACKAGES)
	EchoVar(CC)
	EchoVar(CXX)
	EchoVar(CPPFLAGS)
	EchoVar(CPPFLAGS_DEBUG)
	EchoVar(CPPFLAGS_OPT)
	EchoVar(CFLAGS)
	EchoVar(CFLAGS_DEBUG)
	EchoVar(CFLAGS_OPT)
	EchoVar(CFLAGS_OPT_LESSER)
	EchoVar(CXXFLAGS)
	EchoVar(CXXFLAGS_DEBUG)
	EchoVar(CXXFLAGS_OPT)
	EchoVar(CXXFLAGS_OPT_LESSER)
	EchoVar(C_LDFLAGS)
	EchoVar(C_LDFLAGS_POST)
	EchoVar(CXX_LDFLAGS)
	EchoVar(CXX_LDFLAGS_POST)
	EchoVar(LIBLINK)
	EchoVar(EXEFLAGS)
	EchoVar(COMMON_SYSTEM_LIBS)
	EchoVar(COMMON_SYSTEM_ARCHIVE_LIBS)
	EchoVar(COMMON_SYSTEM_SHLIBS)
	EchoVar(CEDALIBVER)
	EchoVar(SLLIBVER)
	EchoVar(OBJSUFFIX)
	EchoVar(ARSUFFIX)
	...
	EchoVar(LIBRARYTYPE)
	EchoVar(HPEESOF_DIR)
	EchoVar(HPEESOF_DEV_ROOT)
	EchoVar(HPEESOF_DEV_ROOT_UNIX)
	EchoVar(RAW_CDROMDIR)
	EchoVar(CDROMROOT)
	EchoVar(RELEASECDPATH)
	EchoVar(WIN_INCLUDE_VAR)
