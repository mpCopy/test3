# $Header: /cvs/wlv/src/cedamk/mk/m4/win32_64.m4,v 1.27 2010/07/21 22:44:03 build Exp $
...
define(`Win32Flags',` /Zi /D_CRT_SECURE_NO_WARNINGS /D_CRT_NONSTDC_NO_WARNINGS  /favor:blend /D_WIN64 /D_MACHINE:x64  /Wp64  /W4 NoLogo /Dfar=far_p /Dnear=near_p pdbflg')
...
define(`CommonSystemShLibs',`WSock32 kernel32 user32 gdi32 winspool comdlg32 advapi32 shell32 ole32 oleaut32 uuid odbc32 odbccp32')
...
define CCLINKEXE
$(CC_LINKPREFIX) $(LINK) NoLogo Manifest /MACHINE:X64  $(DEBUGOPTION) $(TARGET_C_LDFLAGS) $(C_LDFLAGS) /out:$(EXEPREFIX)$(1) $(2) $(LIBLINK) $(3:%=LibOptionPrefix%LibOptionSuffix) $(TARGET_C_LDFLAGS_POST) $(C_LDFLAGS_POST) $(COMMON_SYSTEM_LIBS:%=LibOptionPrefix%LibOptionSuffix) $(COMMON_SYSTEM_SHLIBS:%=LibOptionPrefix%LibOptionSuffix)
ExePostRule
endef

define CXXLINKEXE
$(CXX_LINKPREFIX) $(LINK) NoLogo Manifest /MACHINE:X64 $(DEBUGOPTION) $(TARGET_CXX_LDFLAGS) $(CXX_LDFLAGS) /out:$(EXEPREFIX)$(1) $(2) $(LIBLINK) $(3:%=LibOptionPrefix%LibOptionSuffix) $(TARGET_CXX_LDFLAGS_POST) $(CXX_LDFLAGS_POST) $(COMMON_SYSTEM_LIBS:%=LibOptionPrefix%LibOptionSuffix) $(COMMON_SYSTEM_SHLIBS:%=LibOptionPrefix%LibOptionSuffix)
ExePostRule
endef')
...
####################
# Dependencies
define(`MakeDependFlags',`-M -S  -- $(filter-out /U%,$(CPPFLAGS:/D%=-D%)) -- $(patsubst /I%,-I%,$(INCLUDEPATHOPTION) $(CINCLUDEPATHOPTION) $(CXXINCLUDEPATHOPTION))  $(WIN_INCLUDE_VAR) ')
