...
LIBCOBJS  =${addprefix $(LIBBASENAME).a(, ${addsuffix ), $(patsubst   %.c, %.o,$(filter %.c, $(SRCS)))}}
ifneq (,$(LIBCOBJS))
(%.o): %.c ;
$(OBJPATH)/$(LIBBASENAME).a :: $(LIBCOBJS)
	${call COMPILE.c, ${addprefix $(SRCPATH)/, $(?:.o=.c)}}
	$(call UPDATE_LIBRARY_COMMAND,$@,${notdir $(?:.c=.o)});
	$(RM) ${notdir $(?:.c=.o)}
endif
LIBCCOBJS =${addprefix $(LIBBASENAME).a(, ${addsuffix ), $(patsubst  %.cc, %.o,$(filter %.cc, $(SRCS)))}}
ifneq (,$(LIBCCOBJS))
(%.o): %.cc ;
$(OBJPATH)/$(LIBBASENAME).a :: $(LIBCCOBJS)
	$(call COMPILE.cxx, $(addprefix $(SRCPATH)/, ${?:.o=.cc}))
	$(call UPDATE_LIBRARY_COMMAND,$@,${notdir $(?:.cc=.o)});
	$(RM) ${notdir $(?:.cc=.o)}
endif
LIBCPPOBJS=${addprefix $(LIBBASENAME).a(, ${addsuffix ), $(patsubst %.cpp, %.o,$(filter %.cpp, $(SRCS)))}}
ifneq (,$(LIBCPPOBJS))
(%.o): %.cpp ;
$(OBJPATH)/$(LIBBASENAME).a :: $(LIBCPPOBJS)
	$(call COMPILE.cxx, $(addprefix $(SRCPATH)/, ${?:.o=.cpp}))
	$(call UPDATE_LIBRARY_COMMAND,$@,${notdir $(?:.cpp=.o)});
	$(RM) ${notdir $(?:.cpp=.o)}
endif
LIBCXXOBJS=${addprefix $(LIBBASENAME).a(, ${addsuffix ), $(patsubst %.cxx, %.o,$(filter %.cxx, $(SRCS)))}}
ifneq (,$(LIBCXXOBJS))
(%.o): %.cxx ;
$(OBJPATH)/$(LIBBASENAME).a :: $(LIBCXXOBJS)
	$(call COMPILE.cxx, $(addprefix $(SRCPATH)/, ${?:.o=.cxx}))
	$(call UPDATE_LIBRARY_COMMAND,$@,${notdir $(?:.cxx=.o)});
	$(RM) ${notdir $(?:.cxx=.o)}
endif
...
# win32
$(OBJPATH)/$(LIBBASENAME)$(SLLIBVER).sl $(LIBBASENAME)$(SLLIBVER).exp: $(OBJS)
	$(call SL_MAKE_EXP,$(LIBBASENAME)..sl,$^,$(LIBBASENAME).exp)
	$(call SL_MAKE_OBJFILES,$(OBJS))
	$(call SL_LINK_CXX,$(OBJPATH)/$(LIBBASENAME)$(SLLIBVER).sl,$(OBJS),$(TARGET_LIBS),$(LIBBASENAME))
...
vpath lib%.a $(LIB_SEARCH_DIRS)
vpath $(LIBPREFIX)%$(SLSUFFIX) $(LIB_SEARCH_DIRS)
$(EXEPREFIX)$(basename $(TARGET)): $(LOCAL_LIBS:%=-l%) $(LIBS:%=-l%)

# DO NOT CHANGE THIS WITHOUT ALSO CHANGING THE SCRIPT THAT HANDLES MULTIPLE
# TARGETS!
$(EXEPREFIX)$(basename $(TARGET)): $(OBJS)
...
MAKEDEPENDFLAGS += -M -- $(CPPFLAGS) -- $(INCLUDEPATHOPTION) $(CINCLUDEPATHOPTION) $(CXXINCLUDEPATHOPTION)
...
$(SRCPATH)/$(MAKE_DEFS_MK):  $(SRCPATH)/make-defs  $(ceda_mk_dir)/bin/multimk
	$(at)$(PERL) $(ceda_mk_dir)/bin/multimk $(TARGETS) > $@
else	# not LONGTARGETSLINE
$(ceda_mk_dir)/bin/multimk: ;
$(SRCPATH)/$(MAKE_DEFS_MK):  $(SRCPATH)/make-defs  $(ceda_mk_dir)/bin/multimk
	$(at)echo "" > $@; for i in $(TARGETS); do $(PERL) $(ceda_mk_dir)/bin/multimk $$i >> $@; done
...
ifeq ($(suffix $(TARGET)),.sl)
ALL_OBJECTS += $(OBJPATH)/$(LIBBASENAME)$(SLLIBVER).sl
INSTALL_OBJECTS += $(INSTALL_LIBDIR)/$(LIBBASENAME).sl
else
ifeq ($(suffix $(TARGET)),.a)
ALL_OBJECTS += $(OBJPATH)/$(LIBBASENAME).a
INSTALL_OBJECTS += $(INSTALL_LIBDIR)/$(LIBBASENAME).a
else
ifeq ($(suffix $(TARGET)),)
ifdef TARGET
ALL_OBJECTS += $(EXEPREFIX)$(basename $(TARGET))
INSTALL_OBJECTS += $(INSTALL_BINDIR)/$(EXEPREFIX)$(basename $(TARGET))
endif	# TARGET
